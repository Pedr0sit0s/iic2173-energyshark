...
export class ForwardService {
   ...
   *   - 4xx → nack(requeue: false): evento inválido, se descarta con log.
   *   - 5xx / timeout / error de red → reintento con backoff; agotado → nack(requeue: false).
   */
    ...
    for (;;) {
      try {
        ...
        if (attempt >= this.maxRetries) {
          channel.nack(message, false, false);
          logger.error(
            `NACK_AGOTADO ${context} reintentos=${attempt} motivo=${reasonOf(error)}`,
          );
          return;
        }
